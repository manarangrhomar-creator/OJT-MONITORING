...
class AdminDashboardViewSet(viewsets.ViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='coordinator-approvals/(?P<user_id>[^/.]+)')
    def set_coordinator_approval(self, request, user_id=None):
        """Update approval status for a coordinator account."""
        status_value = request.data.get('status')
        if status_value not in ['pending', 'approved', 'rejected']:
            return Response({'message': 'Invalid status'}, status=status.HTTP_400_BAD_REQUEST)

        coordinator = get_object_or_404(User, id=user_id, role='coordinator')
